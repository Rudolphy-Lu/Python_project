...
app = Flask(__name__)
app.config["SECRET_KEY"] = "xxxxxxxxxx" # 設置一個金鑰用於加密 session 資料
app.config["SESSION_TYPE"] = "filesystem"  # 將 Session 存儲在伺服器端的檔案系統中
app.config["MAIL_SERVER"] = "smtp.mail.yahoo.com"
app.config["MAIL_PORT"] = 587 # Yahoo Mail 使用的port
app.config["MAIL_USERNAME"] = "xxxxxxxx@yahoo.com"
app.config["MAIL_PASSWORD"] = "xxxxxxxxx" # 此參數為獨有的，不能隨意分享予他人
...
if __name__ == "__main__":
    app.run(debug=False)
    # 單機內測只打開上面那一行，Heroku線上部署則關閉上一行，開啟下方三行
    # import os
